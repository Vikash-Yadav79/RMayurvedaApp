...
//             ...cartItem,
//             quantity:
//               cartItem.quantity + amount > 0 ? cartItem.quantity + amount : 1,
//           }
//         : cartItem,
//     );
...
//         onPress: () => handleMenuAction('Add to Favorites', item),
//       },
...
const CartScreen = () => {
  const {cart, setCart} = useCart(); // Ensure setCart is accessible
  const navigation = useNavigation();
  ...
  // Calculate total amount based on updated quantity and price
  const totalAmount = cart.reduce(
    (sum, item) => sum + parseFloat(item.price) * item.quantity, // Parse the price to ensure it's a number
    0,
  );
